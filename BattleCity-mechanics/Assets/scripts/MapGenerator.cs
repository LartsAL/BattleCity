...
public class MapGenerator : MonoBehaviour
{
    ...
    void GenerateBoundary()
    {
        ...
        for (int x = 0; x < width; x++)
            PlaceAt(x, 0, boundaryPrefab);
        for (int x = 0; x < width; x++)
            PlaceAt(x, height - 1, boundaryPrefab);
        for (int y = 1; y < height - 1; y++)
            PlaceAt(0, y, boundaryPrefab);
        for (int y = 1; y < height - 1; y++)
            PlaceAt(width - 1, y, boundaryPrefab);
    }
        ...
        for (int x = 1; x < width - 1; x++)
        {
            for (int y = 1; y < height - 1; y++)
            {
                if (maze[x, y])
                {
                    ...
                    PlaceAt(x, y, wallPrefabs[idx]);
                }
            }
        }
    }

    void PlaceAt(int x, int y, GameObject prefab)
    {
